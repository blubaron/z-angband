...
int LoadPNG(HWND hWnd, char *filename, HBITMAP *ret_color, HPALETTE *ret_color_palette,
            HBITMAP *ret_mask, HPALETTE *ret_mask_palette,
          	int *ret_wid, int *ret_hgt, bool premultiply);
int SavePNG(HWND hWnd, char *filename, HBITMAP color, HPALETTE color_palette,
            HBITMAP mask, HPALETTE mask_palette,
          	int wid, int hgt, bool unpremultiply);

int ReadTiles(HWND hWnd, char* filename, win32Tilesheet *tiles)
{
...
int ResizeTiles(HWND hWnd, XTilesheet *dest, XTilesheet *src)
{
	if (!(src->color)) {
		return-1;
	}
...
int SaveTiles(HWND hWnd, char* filename, XTilesheet *tiles)
{
