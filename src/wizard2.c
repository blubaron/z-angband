...
static void wiz_display_item(const object_type *o_ptr)
{
	...
	prtf(j + 32, 17,"+------------FLAGS4-------------\n"
			"swwe S  IMSH p pt reHURT..  CURS\n"
			"adds f  ldac alao exaefcld  as h\n"
			"vswy e  iacomtusuptpclioia  utee\n"
			"1hle a  trilurcscsrlierltr  taaa\n"
			"0tkn r  ekddtnkwhinodcedek  ottl\n"
...
static void wiz_create_item(void)
{
	...
	/* Place the object */
	place_specific_object(p_ptr->px, p_ptr->py, p_ptr->depth, k_idx, FALSE);
...
static void do_cmd_wiz_town_teleport(void)
{
	place_type *pl_ptr2;
	int i, j;
